...
**例外**：纯基础设施改动（不改视觉但改 DOM 锚点），如阶段 1 的 §1.0「接 visual-state selector」——main 还没有 selector，before 跑不到。这类改动只产出 after，作为后续依赖它的视觉改动的 **baseline**（存到 `docs/visual/baseline/`，可入 git）。后续改动跳过 before 截取，直接拿 baseline 当 before 对比。

> 触发条件：本项改动只动 `data-*`、ARIA 属性、骨架渲染等"为了让脚手架能截图"的代码，且 main 上不存在对应 selector。其他视觉改动一律走完整 before/after。
...
> **截图等待策略（所有状态都要等 selector）**：Playwright 的 `waitUntil: "load"` 只保证文档资源加载完，**不保证 `/api/data` 已返回、不保证 React 已 render normal/empty UI**——`useDataset()` 的数据走 `useEffect → fetchDataset()` 异步链路（`src/hooks/use-dataset.ts:40`），`load` 触发时 ViewModel 还在 loading 态。截到的会是 skeleton 或空白。
>
> 正确做法是**给每个状态都绑一个语义 selector**，截图脚本统一 wait：
>
> | 状态 | `waitUntil` | wait selector |
> |---|---|---|
> | `normal` | `"load"` | `[data-visual-state="normal"]` |
> | `empty` | `"load"` | `[data-visual-state="empty"]` |
> | `loading` | `"domcontentloaded"` | `[data-visual-state="loading"]` |
>
> selector 由 `data-visual-state` 属性绑在**页面根容器**——按 ViewModel 的当前态切换：
> - 正常数据：`<div data-visual-state="normal">...`
> - ViewModel 返回空数据集：`<div data-visual-state="empty">...`
> - `vm.loading === true`：渲染 `<DashboardSkeleton data-visual-state="loading" />`
>
> 这样 normal/empty 等于"等数据真的渲染上屏才截"，避免 race。`loading` 用 `domcontentloaded`（请求挂着，等不到 `load`）但仍要等 selector，因为 React hydration 可能晚于 DOM 解析。
...
- [ ] **`scripts/visual-snapshot.ts`** —— 参数 `--change <id> --pages <list> --states <list>`：
  ...
  - 对每个 (page, viewport, theme, state) 组合：
    ...
    - **等待规则按状态查表**（详见 §3.5.3）：
      ...
      | `empty` | `"load"` | `[data-visual-state="empty"]` |
      | `loading` | `"domcontentloaded"` | `[data-visual-state="loading"]` |
    - **不要省略 waitForSelector**：所有状态都要等——`load` 不保证 `/api/data` 已返回，hydration 也可能晚于 DOM 解析；不要用 `"networkidle"`，loading 请求永远挂着会超时。
...
### 阶段 0（一天 — 视觉验收脚手架，前置）✅ 已完成 2026-06-13

详见 §3.5。**没做完之前，所有视觉改动都不能开工**——因为没法做改前/改后对比。

- [x] `src/proxy.ts` 加 `WOOLY_VISUAL_BYPASS_AUTH` dev-only bypass（§3.5.1）。
- [x] `src/app/api/data/route.ts` 加 `WOOLY_USE_MOCK` server-side 分支 + `?_visual=` 状态分发（§3.5.2 + §3.5.3）；client/`fetchDataset()` 不动。
- [x] `src/data/datasets.ts` 的 `getDataset()` 加 `state` 参数（normal/empty）。
- [x] 装 `@playwright/test` + chromium。
- [x] `scripts/visual-snapshot.ts` 截图脚本（loading 状态用 `waitUntil: "domcontentloaded"`，主题切换 + before/after 目录）。
- [x] `docs/visual/README.md` + 精确的 `.gitignore`。

### 阶段 1（一天 — 低风险对齐 + loading 接线）✅ 已完成 2026-06-13

阶段 1 的关键依赖：截图脚本要按 §3.5.3 的 selector 表等到 `[data-visual-state]` 才能稳定截图。**当前 main 没有这个 selector**，所以"接 selector"必须先于任何带状态矩阵的改动落地，且它本身的截图基线特殊处理。

按下面的顺序执行：

#### 1.0 — 接 visual-state selector（独立 change-id `1.0`）✅

- [x] 改 `src/app/(dashboard)/page.tsx`：根容器加 `data-visual-state` 属性，按 `vm.loading` / 数据空否切 `loading` / `empty` / `normal`；`vm.loading === true` 时渲染 `<DashboardSkeleton data-visual-state="loading" />`（已存在但从未被引用）。
- [x] tracker / sources 同步加 selector（即使阶段 1 不动 segment，selector 是基础设施）。
- **截图特例**：这是一项**不可视的 selector 接线**，本身不改视觉，所以：
  - **不需要 before/after 对比**——before 跑不到（main 没 selector，screenshot 脚本 wait 会超时），after 截一组 `states=[normal, empty, loading]` × 2 viewport × 2 theme = 12 张验证 selector 工作即可。
  - 这 12 张同时也作为后续 §1.1 / §1.2 / §1.3 改动的 **before 基线**——存到 `docs/visual/baseline/dashboard__normal/empty/loading__*.png`，避免每个改动都重跑 main 状态。
- 实现 + 自查 OK 后再继续后续视觉改动。

#### 1.1 — DashboardSegment（change-id `2.1`）✅

- [x] **新增 `DashboardSegment` 组件**（§2.1 响应式版本，不是 pew 原版）。
- [x] **应用到 `dashboard/page.tsx`**，先切 2-3 段（概览 / 关注 / 分析），不要直接切 4 段。
- 截图矩阵：`pages=[dashboard]`，`states=[normal, empty, loading]`。before 用 §1.0 产出的 baseline；after 跑完整 12 张。

#### 1.2 — 侧边栏分组标题（change-id `2.2`）✅

- [x] `AppSidebar.tsx:70` 单行改：`text-xs font-medium tracking-[0.15em] text-muted-foreground/70`。
- 截图矩阵：`pages=[dashboard]`，`states=[normal]`，4 张。

#### 1.3 — version chip（change-id `2.7-version`）✅

- [x] **新增 `src/lib/version.ts`** + sidebar version chip（§2.7 落地路径）——独立于字体决策，但**不是零成本**，需要验 client bundle 不带整个 package.json。
- 截图矩阵：`pages=[dashboard]`，`states=[normal]`，4 张。

### 阶段 2（1-2 天 — 系统化对齐）✅ 已完成 2026-06-14

- [x] **抽 `SegmentedControl` 组件**（`src/components/ui/segmented-control.tsx`），严格按 §2.3 的"互斥模式 toggle ≤4 项"规则使用；不替换 MemberFilterBar / settings 导航 / tracker 撤销。
- [x] **`StatCardWidget` 加 `variant` + `accentColor`**（默认值向后兼容；dashboard 主指标用 primary）。
- [x] **DashboardSegment 推到 `tracker/page.tsx` 和 `sources/page.tsx`**——前提是阶段 1 dashboard 的截图矩阵通过。
- [x] **统一 Badge 语言**：把 `BenefitStatusBadge` 改成 `Badge` 的薄包装（§2.6 第一步）；`Badge` 加 `info` variant；评估 `SourceCard` 内联 chip 是否能合并（不能就文档化例外）。

### 阶段 3（半周到一周 — 战略品牌探索）✅ 已完成 2026-06-14

- [x] **品牌字体探索**：候选矩阵在 `docs/visual/3.1-wordmark/`（Inter bold / Caveat / Pacifico / Kalam × light/dark × collapsed/expanded）。**结果：选 B · Caveat**——已落地到 `src/app/layout.tsx`（next/font/google + `--font-caveat`）、`src/app/globals.css`（`font-handwriting` utility）、`src/components/AppSidebar.tsx`（`text-[31px] font-bold font-handwriting tracking-tighter mt-[-12px]`）。Web font 成本约 +25 KB。
- [x] **ThemeToggle 简化为 2 态**：`ThemeToggleSimple`（sun ↔ moon）已上 header + login chrome；Settings → 偏好设置 保留完整 3 态 `ThemeToggle`（含 system follow）。7 项单测覆盖切换 + system 覆盖路径。
