...
### 阶段 3（半周到一周 — 战略品牌探索）🟡 候选已出，待哥定字体

- [x] **品牌字体探索**：候选矩阵已产出在 `docs/visual/3.1-wordmark/`——4 个候选（Inter bold / Caveat / Pacifico / Kalam）× light/dark × collapsed/expanded sidebar 头部并置呈现，附 `scripts/wordmark-snapshot.ts` 一次性 Playwright 截图脚本。**`src/` 暂未改动，等哥选定方向后再动 `layout.tsx` 引字体 + `AppSidebar.tsx` 改 className**。
