...
> **截图 loading 状态的等待策略**：Playwright 的 `page.goto()` 默认 `waitUntil: "load"` 会等到 `/api/data` 返回——loading 模式下这个请求永远挂着，goto 必然超时。**截图脚本对 loading 状态必须用 `page.goto(url, { waitUntil: "domcontentloaded" })`**，让 DOM 完成解析就开拍，别等 network idle。其他状态用默认值即可。
...
- [ ] **`scripts/visual-snapshot.ts`** —— 参数 `--change <id> --pages <list> --states <list>`：
  ...
  - 对每个 (page, viewport, theme, state) 组合：
    ...
    - **`loading` 状态用 `page.goto(url, { waitUntil: "domcontentloaded" })`**——其他状态用默认 `"load"`；不要用 `"networkidle"`，否则 loading 请求永远挂着会等到超时；
...
- [ ] **新增 `DashboardSegment` 组件**（§2.1 响应式版本，不是 pew 原版）。**只应用到 `dashboard/page.tsx`**，先切 2-3 段（概览 / 关注 / 分析），不要直接切 4 段——避免和 widget 自带标题形成双层小标题。截图矩阵 `states=[normal, empty, loading]`。
