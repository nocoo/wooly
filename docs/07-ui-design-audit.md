...
- [ ] 改 `src/app/(dashboard)/page.tsx`：根容器加 `data-visual-state` 属性，按 `vm.loading` / 数据空否切 `loading` / `empty` / `normal`；`vm.loading === true` 时渲染 `<DashboardSkeleton data-visual-state="loading" />`（已存在但从未被引用）。
- [ ] tracker / sources 同步加 selector（即使阶段 1 不动 segment，selector 是基础设施）。
...
- [ ] **新增 `DashboardSegment` 组件**（§2.1 响应式版本，不是 pew 原版）。
- [ ] **应用到 `dashboard/page.tsx`**，先切 2-3 段（概览 / 关注 / 分析），不要直接切 4 段。
- 截图矩阵：`pages=[dashboard]`，`states=[normal, empty, loading]`。before 用 §1.0 产出的 baseline；after 跑完整 12 张。

#### 1.2 — 侧边栏分组标题（change-id `2.2`）

- [ ] `AppSidebar.tsx:70` 单行改：`text-xs font-medium tracking-[0.15em] text-muted-foreground/70`。
- 截图矩阵：`pages=[dashboard]`，`states=[normal]`，4 张。

#### 1.3 — version chip（change-id `2.7-version`）

- [ ] **新增 `src/lib/version.ts`** + sidebar version chip（§2.7 落地路径）——独立于字体决策，但**不是零成本**，需要验 client bundle 不带整个 package.json。
- 截图矩阵：`pages=[dashboard]`，`states=[normal]`，4 张。
